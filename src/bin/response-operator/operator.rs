...
use response::istio::destinationrules_networking_istio_io_v1beta1::DestinationRule;
use response::istio::virtualservices_networking_istio_io_v1beta1::VirtualService;
use std::sync::Arc;
use thiserror::Error;
use tokio::time::{Duration, Instant};
use tracing::*;

static SERVICE_FINALIZER_NAME: &str = "response.mt165.co.uk/Service";
...
pub async fn get_controller() -> Result<impl futures::future::Future, kube::Error> {
    let client = response::get_k8s_client().await?;
    ...
    let reporter = Reporter {
        controller: crate::NAME.to_owned(),
    ...
    let c = Controller::new(svc_api, Default::default())
        .owns(dr_api, Default::default())
        .owns(vs_api, Default::default())
        .shutdown_on_signal()
        .run(
            reconcile,
            error_policy,
            Arc::new(ControllerCtx {
                client,
                metrics: Metrics::new(),
                event_reporter: reporter,
            }),
...
async fn reconcile(svc: Arc<Service>, ctx: Arc<ControllerCtx>) -> Result<Action, Error> {
    ...
    let recorder = Recorder::new(
        ctx.client.clone(),
        ctx.event_reporter.clone(),
        svc.object_ref(&()),
    );
    recorder
        .publish(Event {
            type_: EventType::Normal,
            reason: "Created response".to_owned(),
            note: Some("Creating DestinationRule and VirtualService".to_owned()),
            action: "Creating response resources".to_owned(),
            secondary: None,
        })
        .await
...
async fn delete(
    ...
    info!(
        ...
        "DestinationRule and VirtualService deleted by finalizer",
    );
