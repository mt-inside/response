...
use prometheus::{
    default_registry, register_histogram_vec, register_int_counter, Encoder, HistogramVec,
    ...
    pub fn new() -> Self {
        Metrics {
            reconciliations: register_int_counter!(
                format!("{}_reconciliations_total", crate::NAME.replace('-', "_")),
                "reconciliations"
            )
            .unwrap(),
            failures: register_int_counter!(
                "response_operator_failures_total",
                "reconciliation failures"
            )
            .unwrap(),
            reconcile_durations: register_histogram_vec!(
                "response_operator_reconcile_duration_seconds",
